...
export function Workspace({ data, viewerId, selectedId, query, mobileView, speaking, onQueryChange, onMobileViewChange, onSelect, onMakeViewer, onAdd, onCanvasEdit, onDelete, onRoster, onEditProfile, onSaveCustomTerm, onSpeak, onEditRelations, onTimeline }: {
  data: FamilyData
  viewerId: string
  selectedId: string
  query: string
  ...
  const filtered = data.people.filter((person) => person.name.includes(query) || person.aliases?.some((alias) => alias.includes(query)) || person.branch?.includes(query) || calculateKinship(data, viewerId, person.id).mandarin.some((term) => term.includes(query)))
  ...
  return <main className={`workspace mobile-view-${mobileView}`}>
    <aside className="people-panel">
      ...
      <label className="search"><Icon name="search"/><input value={query} onChange={(event) => onQueryChange(event.target.value)} placeholder="搜索姓名、别名、支系或称呼"/></label>
    ...
    <aside className="detail-panel">
      <div className="detail-profile"><Avatar person={selected} size="large"/><div className="profile-copy"><span className="eyebrow">当前查看</span><h2>{selected.name}{selected.living === false && <em className="deceased-mark">故</em>}</h2>{selected.aliases?.length ? <small className="person-aliases">又名：{selected.aliases.join('、')}</small> : null}<p>{birthdaySummary(selected)}{selected.deathDate ? ` — ${selected.deathDate}` : ''} · {formatZodiac(selected.birthYear)}</p>{selected.birthDate && <p className="lunar-birthday">农历：{formatLunarBirthday(selected.birthDate)}</p>}{(selected.hometown || selected.branch) && <p className="profile-origin">{[selected.hometown, selected.branch].filter(Boolean).join(' · ')}</p>}</div><button className="edit-profile-button" onClick={onEditProfile} aria-label={`编辑${selected.name}的资料`}><Icon name="edit"/>编辑</button></div>
      ...
      <button className="manage-relations-button timeline-entry-button" onClick={() => onTimeline(selected.id)}><Icon name="history"/><span><strong>人物生平时间线</strong><small>{selected.events?.length ? `已记录 ${selected.events.length} 个家庭事件` : '记录迁居、求学、职业与家庭记忆'}</small></span></button>
