...
export function FamilyGraph({ data, viewerId, selectedId, onSelect, onMakeViewer, onAdd, onEdit, onDelete }: {
  ...
  const [threeGenerationView, setThreeGenerationView] = useState(false)
  const previousThreeGenerationRef = useRef(false)
  const [legalFilter, setLegalFilter] = useState<LegalFilterId | null>(null)
  const [branchFilter, setBranchFilter] = useState<string | null>(null)
  const [filterMenuOpen, setFilterMenuOpen] = useState(false)
  const [filterCategory, setFilterCategory] = useState<'generation' | 'branch' | 'legal'>('generation')
  ...
  const branchOptions = useMemo(() => [...new Set(data.people.map((person) => person.branch).filter((value): value is string => Boolean(value)))].sort(), [data.people])
  const branchIds = useMemo(() => new Set(branchFilter ? data.people.filter((person) => person.branch === branchFilter).map((person) => person.id) : []), [branchFilter, data.people])
  ...
  const visibleIds = useMemo(() => {
    if (!threeGenerationView) return new Set(data.people.map((person) => person.id))
    ...
    while (queue.length) {
      ...
      const neighbors = [
        ...data.parents.flatMap((edge) => edge.parentId === current ? [edge.childId] : edge.childId === current ? [edge.parentId] : []),
        ...data.spouses.flatMap((edge) => edge.personAId === current ? [edge.personBId] : edge.personBId === current ? [edge.personAId] : []),
      ]
      neighbors.forEach((id) => { if (!distances.has(id)) { distances.set(id, distance + 1); queue.push(id) } })
    }
    return new Set(distances.keys())
  }, [data.parents, data.people, data.spouses, selectedId, threeGenerationView])
  const visiblePeople = useMemo(() => data.people.filter((person) => visibleIds.has(person.id)), [data.people, visibleIds])

  const fitView = useCallback(() => {
    const viewport = viewportRef.current
    if (!viewport || visiblePeople.length === 0) return
    const left = Math.min(...visiblePeople.map((person) => person.x))
    const right = Math.max(...visiblePeople.map((person) => person.x + CARD_W))
    const top = Math.min(...visiblePeople.map((person) => person.y))
    const bottom = Math.max(...visiblePeople.map((person) => person.y + CARD_H))
  ...
  useEffect(() => {
    if (previousThreeGenerationRef.current === threeGenerationView && !threeGenerationView) return
    previousThreeGenerationRef.current = threeGenerationView
    fitView()
  }, [fitView, selectedId, threeGenerationView])
  ...
  data.parents.filter(({ parentId, childId }) => visibleIds.has(parentId) && visibleIds.has(childId)).forEach(({ parentId, childId }) => {
  ...
  const parentLines = [...parentGroups.entries()].map(([key, { parentIds, childIds }]) => {
    ...
    const branchFaded = branchFilter !== null && memberIds.every((id) => !branchIds.has(id) && id !== viewerId)
    const focused = memberIds.includes(selectedId)
    const connectionState = `${faded ? 'generation-faded' : ''} ${legalFaded || branchFaded ? 'view-filtered-out' : ''} ${relationshipFocus ? (focused ? 'relationship-focused' : 'relationship-muted') : ''}`
  ...
  const spouseLines = data.spouses.filter(({ personAId, personBId }) => visibleIds.has(personAId) && visibleIds.has(personBId)).map(({ personAId, personBId }) => {
    ...
    const branchFaded = branchFilter !== null && !branchIds.has(personAId) && !branchIds.has(personBId) && personAId !== viewerId && personBId !== viewerId
    const focused = personAId === selectedId || personBId === selectedId
    const [left, right] = a.x <= b.x ? [a, b] : [b, a]
    return <path key={`${personAId}-${personBId}`} className={`spouse-line ${faded ? 'generation-faded' : ''} ${legalFaded || branchFaded ? 'view-filtered-out' : ''} ${relationshipFocus ? (focused ? 'relationship-focused' : 'relationship-muted') : ''}`} d={`M ${left.x + CARD_W - CONNECTION_OVERLAP} ${left.y + CARD_H / 2} L ${right.x + CONNECTION_OVERLAP} ${right.y + CARD_H / 2}`} />
  })
  ...
  >
    <div className={`graph-stage ${cameraSettling ? 'camera-settling' : ''}`} style={{ transform: `translate3d(${camera.x}px, ${camera.y}px, 0) scale(${camera.scale})`, '--canvas-scale': camera.scale } as CSSProperties}>
      ...
      {visiblePeople.map((person) => {
        ...
        const branchFilteredOut = branchFilter !== null && !branchIds.has(person.id) && !isViewer
    ...
    <div className="quick-filter">
      <div className="quick-filter-triggers" aria-label="画布快速筛选">
        <button className={`quick-filter-trigger focus-filter-trigger ${relationshipFocus ? 'active' : ''}`} type="button" aria-pressed={relationshipFocus} onClick={() => { const next = !relationshipFocus; setRelationshipFocus(next); if (next) { setGenerationView(null); setLegalFilter(null); setBranchFilter(null); setFilterMenuOpen(false); fitView() } }} title={relationshipFocus ? `已突出与当前人物直接相连的 ${Math.max(0, focusIds.size - 1)} 人，点击关闭` : '突出与当前人物直接相连的人物'}>
          <span>聚焦</span>
        </button>
        <button className={`quick-filter-trigger focus-filter-trigger ${threeGenerationView ? 'active' : ''}`} type="button" aria-pressed={threeGenerationView} onClick={() => { setThreeGenerationView((current) => !current); setGenerationView(null); setLegalFilter(null); setBranchFilter(null); setRelationshipFocus(false); setFilterMenuOpen(false) }} title="只显示与当前人物相距三层以内的支系">
          <span>上下三代</span>{threeGenerationView && <em>{visiblePeople.length}</em>}
        </button>
        ...
        <button className={`quick-filter-trigger ${branchFilter ? 'active' : ''}`} type="button" disabled={!branchOptions.length} aria-haspopup="menu" aria-expanded={filterMenuOpen && filterCategory === 'branch'} onClick={(event) => { setFilterAnchor(event.currentTarget); setFilterCategory('branch'); setFilterMenuOpen((current) => filterCategory !== 'branch' || !current) }}>
          <span>支系</span>{branchFilter && <em>{branchIds.size}</em>}<svg viewBox="0 0 24 24" aria-hidden="true"><path d="m7 10 5 5 5-5"/></svg>
        </button>
      ...
      <AdaptiveOverlay open={filterMenuOpen} anchor={filterAnchor} onClose={() => setFilterMenuOpen(false)} label={`${filterCategory === 'generation' ? '辈分' : filterCategory === 'branch' ? '支系' : '法律'}筛选`} menuClassName="quick-filter-menu" sheetClassName="filter-action-sheet" reserved={{ top: 56, right: 8, bottom: 64, left: 8 }} sheetHeader={<div className="sheet-section-heading"><Icon name="filter"/><span><strong>筛选画布人物</strong><small>选择一种查看范围</small></span></div>}>
        {filterCategory === 'generation' ? <div className="view-options">
        ...
        </div> : filterCategory === 'branch' ? <div className="view-options"><div className="filter-menu-heading"><span>按家族支系筛选</span><small>突出人物档案中登记的所属支系</small></div>{branchOptions.map((branch) => <button key={branch} type="button" role="menuitemradio" className={branchFilter === branch ? 'active' : ''} aria-checked={branchFilter === branch} onClick={() => { setBranchFilter(branch); setLegalFilter(null); setGenerationView(null); setRelationshipFocus(false); setFilterMenuOpen(false) }}><span><b>{branch}</b><small>{data.people.filter((person) => person.branch === branch).length} 位人物</small></span><i>{branchFilter === branch ? '✓' : ''}</i></button>)}</div> : <div className="view-options">
          ...
          {legalFilterOptions.map((option) => <button key={option.id} type="button" role="menuitemradio" aria-checked={legalFilter === option.id} className={legalFilter === option.id ? 'active' : ''} onClick={() => { setLegalFilter(option.id); setBranchFilter(null); setGenerationView(null); setRelationshipFocus(false); setFilterMenuOpen(false) }}><span><b>{option.label}</b><small>{option.description}</small></span><i>{legalFilter === option.id ? '✓' : ''}</i></button>)}
          <div className="filter-data-note">“家庭成员”需要共同生活资料，暂不自动判定</div>
        </div>}
        {(legalFilter || branchFilter || generationView !== null || threeGenerationView) && <button className="clear-quick-filter" type="button" onClick={() => { setLegalFilter(null); setBranchFilter(null); setGenerationView(null); setThreeGenerationView(false); setFilterMenuOpen(false) }}>显示全部人物</button>}
      </AdaptiveOverlay>
    </div>
